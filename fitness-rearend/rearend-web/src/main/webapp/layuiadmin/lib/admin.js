...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,sendAuthCode: function(options){
      ...
      var seconds = options.seconds,btn = $(options.elem),token = null
      ...
      btn.on('click', function(){
        var elemPhone = options.elemPhone,value = elemPhone.val();	
        var elemVercode = options.elemVercode, elemVal =elemVercode.val(); 
        ...
        // 获取验证码是否先填写
        if($.trim(elemVal) == '' || $.trim(elemVal).length == 0 || elemVal == null){
        	elemVercode.focus();
        ...
        admin.req($.extend(true, {	 // 具体业务逻辑
          ...
          ,success: function(res){
            options.elemVercode.focus();// 发送短信后聚焦
            countDown();				// 短信发送后进行倒计时
            success && success(res);
          }
        }, options.ajax));
      });
  ...
  var events = admin.events = {
    ...
    ,note: function(othis){
      ...
      events.note.index = admin.popup({
        ...
        ,content: '<textarea placeholder="本地小便签，只限于本机使用哦~ 试试吧，下次登录还能在的呢！"></textarea>'
        ,resize: false
